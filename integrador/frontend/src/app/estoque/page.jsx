...
export default function GestaoEstoque() {
  ...
  return (
    <div className="flex min-h-screen bg-[#121212] text-white overflow-x-hidden font-sans">
     
      <aside className="sticky top-0 z-50 h-screen w-64 bg-[#121212] border-r border-gray-800 flex-shrink-0 transition-all duration-300">
        <SidebarNav />
      </aside>
      ...
      <main className="flex-1 min-w-0 p-4 md:p-8 overflow-y-auto">
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-10">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">Controle de ativos e ferramentas de alta precisão</p>
          </div>
          <button
            onClick={() => {
            ...
            className="w-full lg:w-auto bg-[#7033ff] px-6 py-2.5 rounded-md font-medium text-sm flex items-center justify-center gap-2 cursor-pointer hover:bg-[#8247ff] active:scale-95 transition-all"
          >
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <div className="flex bg-[#1e1e1e] p-1 rounded-lg border border-gray-800 w-fit self-end md:self-auto">
            <button onClick={() => setViewMode("grid")} className={`p-2 rounded-md cursor-pointer transition-all ${viewMode === "grid" ? "bg-[#7033ff] text-white" : "text-gray-400 hover:text-white"}`}><LayoutGrid size={18}/></button>
            <button onClick={() => setViewMode("list")} className={`p-2 rounded-md cursor-pointer transition-all ${viewMode === "list" ? "bg-[#7033ff] text-white" : "text-gray-400 hover:text-white"}`}><List size={18}/></button>
          </div>
        </div>
        ...
        {viewMode === "grid" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
            {ferramentasFiltradas.map((item) => (
              <div key={item.id} className="bg-[#1e1e1e] border border-gray-800 rounded-xl overflow-hidden group hover:border-gray-700 transition-all shadow-lg">
                <div className="relative h-48 bg-[#121212] flex items-center justify-center p-0 overflow-hidden">
                  {item.imagem ? (
                ...
                <div className="p-5">
                  <h3 className="font-bold text-base text-white mb-1 truncate">{item.nome}</h3>
          ...
          <div className="bg-[#1e1e1e] rounded-xl border border-gray-800 overflow-x-auto shadow-lg">
              ...
              <tbody className="divide-y divide-gray-800">
                {ferramentasFiltradas.map((item) => (
                  <tr key={item.id} className="hover:bg-[#252525] transition-colors">
                    ...
                    <td className="px-6 py-4 text-gray-300 text-sm">{item.responsavel}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="bg-[#1e1e1e] border border-gray-800 rounded-xl w-full max-w-lg shadow-2xl overflow-hidden">
            <div className="p-6 md:p-8">
              ...
              <form className="space-y-5" onSubmit={handleSalvar}>
                ...
                <div className="flex justify-center mb-6">
                  <label className="relative w-40 h-40 rounded-lg bg-[#121212] border-2 border-dashed border-gray-700 flex flex-col items-center justify-center cursor-pointer hover:border-[#7033ff]/50 hover:bg-[#151515] transition-all overflow-hidden group">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-2">
                    ...
                    <select
                      ...
                      className="bg-[#121212] border border-gray-800 rounded-md p-2.5 text-sm text-white outline-none focus:border-[#7033ff] cursor-pointer"
                    >
...
{/* FUNÇÃO STATCARD RESTAURADA AO ORIGINAL */}
...
function StatusBadge({ status }) {
  ...
  return <span className={`text-xs px-2.5 py-1 rounded-md font-medium border-none ${styles[status]}`}>{status}</span>;
}
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <input
        ...
        className="bg-[#121212] border border-gray-800 rounded-md p-2.5 text-sm text-white outline-none focus:border-[#7033ff] transition-all"
      />
    </div>
  );
}
