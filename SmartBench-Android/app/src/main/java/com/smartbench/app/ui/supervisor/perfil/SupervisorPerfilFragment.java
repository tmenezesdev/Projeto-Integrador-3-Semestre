...
public class SupervisorPerfilFragment extends Fragment {

    private FragmentAdminPerfilBinding binding;
    private SupervisorPerfilViewModel viewModel;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        viewModel = new ViewModelProvider(this).get(SupervisorPerfilViewModel.class);

        // Supervisor não tem acesso à tela de Configurações
        binding.btnConfiguracoes.setVisibility(View.GONE);

        binding.btnHistorico.setOnClickListener(v ->
                Navigation.findNavController(v).navigate(R.id.action_supervisor_perfil_to_historico));

        binding.btnLogout.setOnClickListener(v -> {
            if (getActivity() instanceof SupervisorHostActivity) {
                ((SupervisorHostActivity) getActivity()).logout();
            }
        });
    ...
    private void populatePerfil(Usuario u) {
        ...
        binding.tvPerfil.setText("Supervisor");
